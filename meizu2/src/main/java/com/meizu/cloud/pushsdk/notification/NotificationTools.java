...
public class NotificationTools {
    ...
    public static Intent buildIntent(Context context, MPushMessage mPushMessage) {
        if ("0".equals(mPushMessage.getClickType())) {
            ...
            return launchIntentForPackage;
        } else if ("1".equals(mPushMessage.getClickType())) {
            String str;
            Object obj = "";
            if (mPushMessage.getParams() != null) {
                for (Entry entry2 : mPushMessage.getParams().entrySet()) {
                    ...
                    if (TextUtils.isEmpty((CharSequence) entry2.getKey()) || TextUtils.isEmpty((CharSequence) entry2.getValue())) {
                        ...
                        str = r2 + "S." + ((String) entry2.getKey()) + "=" + urlEncode((String) entry2.getValue()) + MzGroups.GROUP_SPLIT_MARK_VCARD;
                    }
                    Log.i(TAG, "paramValue " + str);
                    String str2 = str;
                }
            }
            str = "intent:#Intent;component=" + mPushMessage.getPackageName() + MzGroups.GROUP_SPLIT_MARK_SLASH + ((String) mPushMessage.getExtra().get(PushConstants.INTENT_ACTIVITY_NAME)) + (TextUtils.isEmpty(obj) ? MzGroups.GROUP_SPLIT_MARK_VCARD : MzGroups.GROUP_SPLIT_MARK_VCARD + obj) + "end";
        ...
        } else if (!"2".equals(mPushMessage.getClickType())) {
            return null;
        } else {
